...
Такой клиентский код называется **Продюсером (Producer)** он публикуют события в брокер. 
...
___
### Задание. 
Доработайте код: объедените код читателя и писателя. Сделайте, чтобы читатель и писатель могли работать параллельно. А если читатель вдруг по какой-то причине будет выключен, необходимо, чтобы он начал читать с того самого места (**Смещение/Оффсет**), на котором он остановился. Для хранения *смещения* используйте файл
...
2. **Консьюмер (Consumer)** называется приложение, которое подписывается на интересующие его сообщения и обрабатывает их.
...
![Настройка топика](docs/set_topic.png)
...
![отправленные сообщения](docs/send_mes.png)
