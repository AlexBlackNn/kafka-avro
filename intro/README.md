...
Для управления схемами данных, которые используются при сериализации и десериализации сообщений в Kafka (позволяет обеспечить согласованность данных между продьюсерами и консьюмерами) испольузется **Schema Registry** (SR). SR  поддерживает различные форматы сериализации, такие как Avro, JSON и Protobuf.  
